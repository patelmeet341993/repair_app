...
class ServiceCenterDialog extends StatefulWidget {
  ...
  ServiceCenterDialog({required this.service, this.cart, this.cartIndex, this.isFromDetails = false});
                                              ...
                                              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                                Expanded(
                                                  child: Column(
                                                    ...
                                                    children: [
                                                      Text(
                                                        cartControllerInit.initialCartList[index].variantKey.replaceAll('-', ' '),
                                                      ...
                                                      Text(
                                                          PriceConverter.convertPrice(double.parse(cartControllerInit.initialCartList[index].price.toString()),
                                    ...
                                    ? () async {
                                        if (_addToCart) {
                                          ...
                                          if (Get.find<AuthController>().isLoggedIn()) {
                                            await cartController.addCartToServer(widget.service?.id, widget.service?.subCategoryId ?? "", context);
                                            await cartController.getCartListFromServer();
                                            // Get.back();
                                          } else {
                                            cartController.addDataToCart(widget.service?.id, widget.service?.subCategoryId ?? "");
                ...
                children: [
                  Positioned(
                    ...
                    child: Container(
                      ...
                      width: 40,
                      alignment: Alignment.center,
                      decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white70.withOpacity(0.6), boxShadow: [
                        BoxShadow(
