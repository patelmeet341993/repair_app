...
import 'package:repair/feature/location/model/place_details_model.dart';
...
class LocationController extends GetxController implements GetxService {
    ...
    if (_updateAddAddressData) {
      ...
      try {
        ...
        ZoneResponseModel _responseModel = await getZone(
            ...
            true);
        if (formCheckout &&
            !_responseModel.zoneIds.contains(getUserAddress()?.zoneId)) {
          Get.dialog(
            ConfirmationDialog(
                description: null,
                icon: null,
  ...
  Future<ResponseModel> deleteUserAddressByID(AddressModel address) async {
    ResponseModel _responseModel =
        ResponseModel(false, 'address_delete_failed'.tr);
  ...
  Future<void> getAddressList() async {
    ...
    _addressList =
        _addressList != null ? _addressList!.reversed.toList() : null;
    _isLoading = false;
    _addLocalAddress();
    update();
  }
  ...
  _addLocalAddress() {
    ...
    if (getUserAddress() != null) {
      bool _isAddressContain = _addressList != null
          ? _addressList!
              .where((element) => element.address == getUserAddress()?.address)
              .isNotEmpty
          : false;
  ...
  Future<void> addAddress(
      AddressModel addressModel, bool _fromAddAddressScreen) async {
  ...
  Future<bool> saveUserAddress(AddressModel address) async {
    ...
    return await locationRepo.saveUserAddress(
        userAddress, address.zoneId != null ? address.zoneId : null);
  }
  ...
  Future<void> saveAddressAndNavigate(AddressModel address, bool fromSignUp,
      String? route, bool canRoute) async {
    print("save_address_and_navigate");
    ZoneResponseModel _responseModel = await getZone(
        address.latitude.toString(), address.longitude.toString(), true);

    if ((getUserAddress() != null && getUserAddress()!.zoneId != null)
        ? !_responseModel.zoneIds.contains(getUserAddress()!.zoneId)
        : true && Get.find<CartController>().cartList.length > 0) {
      ...
      _setZoneData(address, fromSignUp, route != null ? route : null, canRoute,
          false, _responseModel.zoneIds);
    }
  }

  void _setZoneData(AddressModel address, bool fromSignUp, String? route,
      bool canRoute, bool shouldCartDelete, String? zoneIds) {
    if (zoneIds != null) {
  ...
  Future<AddressModel> setLocation(String placeID, String address,
    ...
    if (response.statusCode == 200) {
      ...
      if (_placeDetails.content!.status == 'OK') {
        _latLng = LatLng(
            _placeDetails.content!.result!.geometry!.location!.lat!,
            _placeDetails.content!.result!.geometry!.location!.lng!);
      }
    }
  ...
  Future<bool> setAddressIndex(AddressModel address,
    ...
    if (fromAddressScreen) {
      ZoneResponseModel _selectedZone =
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    ...
    if (response.statusCode == 200 &&
      ...
      _address = response.body['content']['results'][0]['formatted_address']
          .toString();
    } else {
      customSnackBar(
  ...
  Future<List<PredictionModel>> searchLocation(
    ...
    if (text.isNotEmpty) {
      ...
      if (response.body['response_code'] == "default_200" &&
        ...
        customSnackBar(response.body['message'] ?? response.bodyString,
            isError: false);
      }
    }
