...
class ProductDetailsController extends GetxController {
  ...
  Future<void> getProductDetails(String productID) async {
    ...
    print("response in getProductDetails: ${response.body}");
    if (response.body['response_code'] == 'default_200') {
      if (ParsingHelper.parseListMethod(response.body['content']['data']).isNotEmpty) {
        _product = Product.fromJson(ParsingHelper.parseMapMethod(ParsingHelper.parseListMethod(response.body['content']['data'])[0]));
      } else {
        _product = Product();
      }
    } else {
