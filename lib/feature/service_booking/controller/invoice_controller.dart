...
import 'package:repair/feature/service_booking/controller/product_booking_details_tabs_controller.dart';
...
class ProductInvoiceController {
  static Future<Uint8List> generateUint8List(
      BookingDetailsContent bookingDetailsContent,
      List<InvoiceItem> items,
      ProductBookingDetailsTabsController controller) async {
    final pdf = Document();
    final netImage = await networkImage(
        '${Get.find<SplashController>().configModel.content!.imageBaseUrl}'
        '/business/${Get.find<SplashController>().configModel.content!.logo}');
    ...
    pdf.addPage(MultiPage(
      build: (context) => [
        buildInvoiceImage(netImage, bookingDetailsContent, invoice),
        SizedBox(height: 0.6 * PdfPageFormat.cm),
        invoiceIDSchedule(invoice.info.number, bookingDetailsContent.createdAt ?? ""),
        buildHeader(invoice, bookingDetailsContent),
        SizedBox(height: 0.8 * PdfPageFormat.cm),
        buildInvoice(invoice),
        SizedBox(height: Dimensions.PADDING_SIZE_DEFAULT),
  ...
  static Widget invoiceIDSchedule(String invoiceID, String invoiceSchedule) =>
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text("Invoice # $invoiceID",
            style: pw.TextStyle(fontWeight: FontWeight.bold)),
        Text("Order Delivery Date : ${DateConverter.dateMonthYearTimeTwentyFourFormat(DateConverter.isoUtcStringToLocalDate(invoiceSchedule))}"),
      ]);
  ...
  static Widget buildTotal(BookingDetailsContent bookingDetailsContent,
      ProductBookingDetailsTabsController controller, String paymentStatus) {
    double _serviceDiscount = 0;
    bookingDetailsContent.detail?.forEach((service) {
      _serviceDiscount = _serviceDiscount + service.discountAmount!;
    });
            ...
            child: Column(
              ...
              children: [
                ...
                buildText(
                  ...
                  value: controller.productBookingDetailsContent!.totalBookingAmount!
                      .toStringAsFixed(2),
                  unite: true,
                ),
                SizedBox(height: 2 * PdfPageFormat.mm),
