...
class ProductBookingStatusTabItem extends GetView<ServiceBookingController> {
  const ProductBookingStatusTabItem({Key? key, required this.title}) : super(key: key);
  final String title;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      padding: EdgeInsets.symmetric(
          horizontal: Dimensions.PADDING_SIZE_DEFAULT, vertical: 5),
      decoration: BoxDecoration(
          color: controller.selectedProductBookingStatus.name != title
              ? Colors.grey.withOpacity(0.2)
              : Theme.of(context).colorScheme.primary,
          borderRadius: BorderRadius.all(Radius.circular(20))),
      child: Row(
