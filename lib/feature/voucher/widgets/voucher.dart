  ...
  void onAddTap(){
    if (Get.find<AuthController>().isLoggedIn()) {
      ...
      if (_addCoupon) {
        ...
        Get.find<CouponController>().applyCoupon(couponModel).then(
              (value) {
                    ...
                    Row(
                      ...
                      children: [
                          ...
                          children: [
                            Text(
                              ...
                              style: ubuntuRegular.copyWith(
                                  color: Theme.of(context).textTheme.bodyText1!.color!.withOpacity(.6), fontSize: Dimensions.fontSizeSmall),
                            ),
                            SizedBox(
                        ...
                        InkWell(
                          ...
                          child: Container(
                            decoration: BoxDecoration(
                                color: Theme.of(context).colorScheme.primary, borderRadius: BorderRadius.all(Radius.circular(Dimensions.RADIUS_SMALL))),
                            child: Center(
                              child: Text(
                                'use'.tr,
                                style: ubuntuRegular.copyWith(
...
class ProductVoucher extends StatelessWidget {
  final CouponModel couponModel;

  const ProductVoucher({Key? key, required this.couponModel}) : super(key: key);

  void onAddTap(){
    if (Get.find<AuthController>().isLoggedIn()) {
      bool _addCoupon = false;
      Get.find<ProductCartController>().cartList.forEach((_cart) {
        print("Cart price: ${_cart.price} Discount Price ${couponModel.discount!.minPurchase!.toDouble()}");

        if (_cart.price > couponModel.discount!.minPurchase!.toDouble()) {
          _addCoupon = true;
        }
      });
