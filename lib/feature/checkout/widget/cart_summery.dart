...
import 'package:repair/feature/checkout/widget/row_text.dart';
...
import '../../shop/features/cart/controller/product_cart_controller.dart';
...
class CartSummery extends GetView<CartController> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<CouponController>(builder: (_couponControllers) {
      return GetBuilder<CartController>(builder: (cartController) {
        ...
        cartController.cartList.forEach((cartModel) {
          _subTotalPrice = _subTotalPrice +
              (cartModel.serviceCost *
                  cartModel.quantity); //(without any discount and coupons)
          _disCount = _disCount + cartModel.discountedPrice;
          _campaignDisCount =
              _campaignDisCount + cartModel.campaignDiscountPrice;
          _couponDisCount = _couponDisCount + cartModel.couponDiscountPrice;

          _vat = _vat + (cartModel.taxAmount);
        });
        double grandTotal = (_subTotalPrice -
                (_couponDisCount + _disCount + _campaignDisCount)) +
            _vat;
                ...
                child: Column(
                  children: [
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Column(
                          ...
                          children: [
                            RowText(
                                title: cartList.elementAt(index).service!.name!,
                    ...
                    RowText(
                        ...
                        price: _campaignDisCount),
                    RowText(
                        title: 'coupon_discount'.tr, price: _couponDisCount),
...
class ProductCartSummery extends GetView<CartController> {
  const ProductCartSummery({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return GetBuilder<CouponController>(builder: (_couponControllers) {
      return GetBuilder<ProductCartController>(builder: (cartController) {
        List<CartProductModel> cartList = cartController.cartList;
        ...
        cartController.cartList.forEach((cartModel) {
          ...
          _disCount = _disCount + cartModel.discountedPrice;
          _campaignDisCount =
              _campaignDisCount + cartModel.campaignDiscount;
                ...
                child: Column(
                  children: [
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        double totalCost =
                            cartList.elementAt(index).price.toDouble() *
                                cartList.elementAt(index).quantity;
                        return Column(
                          mainAxisAlignment: MainAxisAlignment.start,
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            RowText(
                                title: cartList.elementAt(index).productName,
                                quantity: cartList.elementAt(index).quantity,
                                price: totalCost),
                            Container(
                              width: Get.width / 2.5,
                              child: Text(
                                "${cartList.elementAt(index).productVariantId}",
                                style: ubuntuMedium.copyWith(
                                    color: Theme.of(context)
                                        .textTheme
                                        .bodyText1!
