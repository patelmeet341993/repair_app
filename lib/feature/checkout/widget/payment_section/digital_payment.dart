...
class DigitalPayment extends StatelessWidget {
  ...
  DigitalPayment(
      {Key? key, required this.paymentGateway, required this.addressId})
      : super(key: key);
    ...
    return GestureDetector(
      onTap: () {
        if (Get.find<CartController>().cartList.length > 0) {
          ...
          String? addressID =
              _addressModel != null ? _addressModel.id.toString() : addressId!;

          if (addressID != '') {
            String _url = '';
            String _schedule = DateConverter.dateToDateOnly(
            ...
            _url =
                '${AppConstants.BASE_URL}/payment/${paymentGateway.replaceAll('_', '-')}/pay?access_token=${base64Url.encode(utf8.encode(_userId))}&&zone_id=${Get.find<LocationController>().getUserAddress()!.zoneId}'
                '&&service_schedule=$_schedule&&service_address_id=$addressID';

            if (GetPlatform.isWeb) {
              _url =
                  '$_url&&callback=$protocol//$hostname:$port${RouteHelper.checkout}';
